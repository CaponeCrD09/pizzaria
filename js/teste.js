...
// Oculta o card original (template) assim que o script carrega para não aparecer vazio
let cardModeloOriginal = document.querySelector('.pizza-card');
if (cardModeloOriginal) {
    cardModeloOriginal.style.display = 'none';
}

// teste para carrega uma nova versao do html 

async function Teste_nova_versao(){
    let pegarteste = document.querySelector('.pizza-card')
    let cloneteste = pegarteste.cloneNode(true)
    document.querySelector('.grid-pizzas').appendChild(cloneteste)
    
}

function renderizarPizza(data) {
    let converter_id = data.id.toString();

    let pegarteste = document.querySelector('.pizza-card');
    if (!pegarteste) return;

    let cloneteste = pegarteste.cloneNode(true);
    cloneteste.style.display = ""; // Garante que o clone seja visível

    // NOME
    let nomeDom = cloneteste.querySelector("h3");
    if (nomeDom) {
        nomeDom.id = "nome_produto_" + converter_id;
        nomeDom.textContent = data.nome;
    }

    // DESCRIÇÃO
    let descricaoDom = cloneteste.querySelector("p");
    if (descricaoDom) {
        descricaoDom.id = "descricao_produto_" + converter_id;
        descricaoDom.textContent = data.descricao;
    }

    // PREÇO
    let precoDom = cloneteste.querySelector(".preco");
    if (precoDom) {
        precoDom.id = "preco_produto_" + converter_id;
        precoDom.textContent = `R$ ${data.preco.toFixed(2).replace('.', ',')}`;
    }

    // BOTÃO
    let btn = cloneteste.querySelector('.btn-pedir');
    if (btn) {
        btn.setAttribute('data-nome', data.nome);
        btn.setAttribute('data-preco', data.preco);
    }

    // Só adiciona na tela depois de tudo estar preenchido
    document.querySelector('.grid-pizzas').appendChild(cloneteste);
}

async function carregarProdutos() {
    // Busca TODAS as pizzas de uma vez só, em ordem de ID
    ...
    if (data) {
        // Renderiza cada pizza encontrada
        data.forEach(pizza => {
...
// Adiciona evento usando delegação (para funcionar com botões criados dinamicamente)
document.addEventListener('click', function (e) {
    if (e.target && e.target.classList.contains('btn-pedir')) {
        const botao = e.target;
        const card = botao.closest('.pizza-info');
        const nomePizza = botao.getAttribute('data-nome');

        // Pega o preço atual
        const precoBase = parseFloat(botao.getAttribute('data-preco'));
        ...
        setTimeout(() => {
            ...
            botao.style.backgroundColor = "#ffc107"; // volta pra cor original
        }, 1500);
    }
});
